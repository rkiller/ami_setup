echo Setup OSSEC
if ! sudo test /var/ossec/bin/ossec-control; then
	...
	sudo mkdir ${WORKSPACE_OSSEC} && sudo tar -vxzf ${WORKSPACE}/ossec.tar.gz -C ${WORKSPACE_OSSEC} --strip-components 1
	...
	sudo wget -O ${WORKSPACE_OSSEC}/pcre2-10.32.tar.gz https://sourceforge.net/projects/pcre/files/pcre2/10.32/pcre2-10.32.tar.gz/download
	sudo tar xzf ${WORKSPACE_OSSEC}/pcre2-10.32.tar.gz -C ${WORKSPACE_OSSEC}/src/external
	
	echo Install OSSEC
	sudo PCRE2_SYSTEM=no ZLIB_SYSTEM=no ${WORKSPACE_OSSEC}/install.sh
